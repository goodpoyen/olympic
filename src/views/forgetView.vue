			...
			<validation-observer ref="observer" v-slot="{ invalid }" v-show='verify'>
                <div class="teal lighten-4 darken-2 text-center">
			  	...
			  	<form @submit.prevent="submit" style="padding: 20px;">
					<validation-provider v-slot="{ errors }" name="信箱" rules="required|email">
						<v-text-field v-model="account" :error-messages="errors" label="信箱" required></v-text-field>
					</validation-provider>
                	<v-btn block class="green mr-4" :disabled="invalid"  @click="send">領取驗證碼</v-btn>
            	</form>
			</validation-observer>
            <validation-observer ref="observer" v-slot="{ invalid2 }" v-show='!verify'>
                <div class="teal lighten-4 darken-2 text-center">
                    <span class="white--text">忘記密碼</span>
                </div>
                <form @submit.prevent="submit" style="padding: 20px;">
                    <validation-provider v-slot="{ errors }" name="驗證碼" rules="required">
						<v-text-field v-model="token" :error-messages="errors" label="驗證碼" required></v-text-field>
					</validation-provider>
                	<v-btn block class="green mr-4" :disabled="invalid2"  @click="goChange">變更密碼</v-btn>
            	</form>
			</validation-observer>
